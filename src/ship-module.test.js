...
test('marks a spot in the ship to be hit', () => {
  const testShip = ship(5)
  testShip.hit(3)
  expect(testShip.placesHit).toContain(3)
})

test('hit does not mark a spot twice', () => {
  const testShip = ship(5)
  testShip.hit(3)
  testShip.hit(3)
...
test('hit does not mark a spot that does not exist', () => {
  const testShip = ship(5)
  testShip.hit(6)
  expect(testShip.placesHit).toEqual([])
})

test('isSunk return false when only 1 place is hit', () => {
  const testShip = ship(5)
  testShip.hit(3)
  expect(testShip.isSunk()).toBeFalsy()
})

test('isSunk return true if all places is hit', () => {
  const testShip = ship(5)
  testShip.hit(0)
  testShip.hit(1)
  testShip.hit(2)
